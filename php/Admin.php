            ...
            if(isset($_POST['save'])){
                ...
                if($email != $rowP['email']) {
                     ...
                     if ($row['type'] !== 'Super-Super-Admin') {
                        ...
                        $org2 = $row['org'];
                        $logo = $row['logo'];
                        $sql = "INSERT INTO users (type, fullname, email, org, phone, address, city,logo, code)"
                             ."VALUES ('$type', '$name','$email','$org2', '$phone','$address', '$city','$logo','$code' )";
                        $res = mysqli_query($conn,$sql);
                            ...
                            $logo_path = $conn->real_escape_string('L_assets/images/logos/'.$_FILES['logo']['name']);
                                ...
                                $acceptable = array(
                                    ...
                                    if(count($errors) === 0){
                                        ...
                                        if(copy($_FILES['logo']['tmp_name'], $logo_path)){
                                                ...
                                                // $sql = "UPDATE users SET logo = '$logo_path' WHERE email ='$email'";
                                                // $res = mysqli_query($conn, $sql);
                                            }
                                    } else{
                            ...
                            $sql = "INSERT INTO users (type, fullname, email, org, phone, address, city, code, logo)"
                                  ."VALUES ('$type', '$name','$email','$org', '$phone','$address', '$city', '$code', '$logo_path')";
                             $res = mysqli_query($conn,$sql);
                   ...
                   <?php if($row['logo'] == ''){ ?>
                            ...
                            <?php }
                                    ...
                                    ?>
                                        ...
                                        <img class = "logoSize" src="<?php echo $row['logo']?>" alt="..."/>
  ...
  <form class="modal modal2" style="width: 100%;" method="post" action="" enctype="multipart/form-data" role="form">
    ...
    <div class="modal--content">
       ...
       <div class="row">
                <div class="col-md-5">
                    ...
                    <?php 
                                 if($row['type'] === 'Super-Super-Admin'){
                                    echo '<select name="type" id="type" onchange="showOrg("#orgDisplay", this)" style="font-size: initial;"> 
    ...
    <div class = "formUpoad">
               <input type="file" name="logo"
                         <?php if($row['type'] !== 'Super-Super-Admin'){
                                     echo 'style="float: right; display:none"';
                                }
                                 ?>>
...
<?php
    if(isset($_POST['save']) ){
            ...
            //     }
            // }else{
            //     //email exist within the database
            // }
...
?>
   ...
   <script src = "assets/js/angular.min.js"  type="text/javascript"></script> 
    ...
    <script>
        function showOrg(divId, element){
            document.getElementById(divId).style.display = element.value == 'Super-Super-Admin' ? 'none' : 'block';
        }
    </script>
